package com.duowan.faceit.router.compiler;
...
public class RouterProcessor extends AbstractProcessor {
    ...
    private void generateDefaultRouterInit() {
        ...
        TypeSpec routerInit = TypeSpec.classBuilder("RouterInit")
                ...
                .build();
        try {
            JavaFile.builder("com.duowan.faceit.router", routerInit)
                    .build()
                    .writeTo(filer);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private boolean handleRouter(String genClassName, RoundEnvironment roundEnv) {
        ...
        MethodSpec.Builder mapMethod = MethodSpec.methodBuilder("map")
                ...
                .addStatement("com.duowan.faceit.router.ExtraTypes extraTypes")
                .addCode("\n");
        ...
        for (Element element : elements) {
            ...
            mapMethod.addStatement("extraTypes = new com.duowan.faceit.router.ExtraTypes()");
            ...
            for (String format : router.value()) {
                ...
                if (element.getKind() == ElementKind.CLASS) {
                    mapMethod.addStatement("com.duowan.faceit.router.Routers.map($S, $T.class, null, extraTypes)", format, className);
                } else {
                    mapMethod.addStatement("com.duowan.faceit.router.Routers.map($S, null, " +
                            "new MethodInvoker() {\n" +
                            "   public void invoke(android.content.Context context, android.os.Bundle bundle) {\n" +
                            "       android.arch.lifecycle.ViewModelProviders.of((android.support.v4.app.FragmentActivity)context).get($T.class).$N(context, bundle);\n" +
                            "   }\n" +
                            "}, " +
        ...
        TypeSpec routerMapping = TypeSpec.classBuilder(genClassName)
                ...
                .build();
        try {
            JavaFile.builder("com.duowan.faceit.router", routerMapping)
                    .build()
                    .writeTo(filer);
        } catch (Throwable e) {
            e.printStackTrace();
