...
public class Mapping {
    ...
    public HashMap<String, Object> parseParams(String url) {
        return parseParams(Uri.parse(url));
    }

    public HashMap<String, Object> parseParams(Uri uri) {
        HashMap<String, Object> params = new HashMap<>();
        Path p = formatPath.next();
        Path y = Path.create(uri).next();
        while (p != null) {
            if (p.isArgument()) {
                putParam(params, p.argument(), y.value());
            }
            p = p.next();
            y = y.next();
        }
        ...
        for (String name : names) {
            String value = uri.getQueryParameter(name);
            putParam(params, name, value);
        }
        return params;
    }

    private void putParam(HashMap<String, Object> params, String name, String value) {
        ...
        switch (type) {
            ...
            case ExtraTypes.LONG:
                params.put(name, Long.parseLong(value));
                break;
            case ExtraTypes.BOOL:
                params.put(name, Boolean.parseBoolean(value));
                break;
            case ExtraTypes.SHORT:
                params.put(name, Short.parseShort(value));
                break;
            ...
            case ExtraTypes.BYTE:
                params.put(name, Byte.parseByte(value));
                break;
            case ExtraTypes.CHAR:
                params.put(name, value.charAt(0));
                break;
            default:
